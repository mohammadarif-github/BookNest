...
TYPE = (
    ...
    def can_be_cancelled(self):
        ...
        return self.status in ['pending', 'payment_pending', 'awaiting_approval', 'confirmed'] and \
               self.checking_date and self.checking_date.date() > timezone.now().date()ned')
)
...
class UserRole(models.Model):
    """
    Extended user roles for hotel management
    """
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='hotel_role')
...
class GuestProfile(models.Model):
    """
    Extended guest profile for hotel management
    """
